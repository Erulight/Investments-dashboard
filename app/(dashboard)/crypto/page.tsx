      ...
      <div className="bg-gradient-to-r from-orange-500 to-yellow-500 rounded-2xl shadow-xl p-8 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-lg text-orange-100">
              Cryptocurrency trading and investment tracking
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-4 border border-white/20">
            <p className="text-sm text-orange-100 mb-1">Total Portfolio Value</p>
            <p className="text-2xl font-bold">${totalValue.toLocaleString()}</p>
          </div>
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-4 border border-white/20">
            <p className="text-sm text-orange-100 mb-1">Total Return</p>
            <p className="text-2xl font-bold">
              ${totalReturn.toLocaleString()}
            </p>
          </div>
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-4 border border-white/20">
            <p className="text-sm text-orange-100 mb-1">Return Percentage</p>
            <p className="text-2xl font-bold">
        ...
        {investments.length === 0 ? (
          <Card>
            <CardContent className="py-20 text-center">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-2">No Crypto Investments Yet</h3>
              <p className="text-gray-500 text-lg">
                {user.role === 'OWNER' 
                  ? 'Start by importing data or creating your first crypto investment.' 
                  : 'Contact the owner to add you to crypto investments.'}
              </p>
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-2xl font-bold text-gray-900">All Crypto Trades</CardTitle>
                  <p className="text-sm text-gray-500 mt-1">{investments.length} active positions</p>
                </div>
                {user.role === 'OWNER' && (
                  <button className="px-6 py-3 bg-gradient-to-r from-orange-500 to-yellow-500 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
                    + Add New Trade
                  </button>
                )}
              </div>
              ...
              <Table>
                ...
                <TableBody>
                  {investments.map((inv: any) => {
                    ...
                    return (
                      <TableRow key={inv.id} className="hover:bg-orange-50 transition-colors duration-150">
