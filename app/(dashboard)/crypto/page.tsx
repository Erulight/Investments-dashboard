      ...
      <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-xl shadow-md p-6 text-white">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Cryptocurrency trading and investment tracking</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="bg-white/5 rounded-lg p-3 border border-white/10">
            ...
            <p className="text-lg font-bold mt-0.5">${totalValue.toLocaleString()}</p>
          </div>
          <div className="bg-white/5 rounded-lg p-3 border border-white/10">
            <p className="text-[11px] text-slate-400 uppercase tracking-wider">Total Return</p>
            <p className="text-lg font-bold mt-0.5">${totalReturn.toLocaleString()}</p>
          </div>
          <div className="bg-white/5 rounded-lg p-3 border border-white/10">
      ...
      {investments.length === 0 ? (
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-1">No Crypto Investments Yet</h3>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-sm font-bold text-gray-800">All Crypto Trades</CardTitle>
                <p className="text-xs text-gray-500 mt-0.5">{investments.length} active positions</p>
              </div>
              {user.role === 'OWNER' && (
